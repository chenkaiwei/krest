...
@Slf4j
@Component
//@ConditionalOnProperty()//TODO 回头确定下webConfig里的代码够不够让这个不自动实现
...
//        String tempAesKey = RSAUtils.privateDecrypt(cryptionString, privateKey);
            ...
            AsymmetricCrypto tempSecretKeyCryptoAlgorithm=krestConfigurer.initTempSecretKeyCryptoAlgorithm();
            Assert.notNull(tempSecretKeyCryptoAlgorithm,"您尚未实现KrestConfigurer.tempSecretKeyCryptoAlgorithm接口");

            byte[] encryptBytes = Base64.decode(cryptionString);

            byte[] decrypt = tempSecretKeyCryptoAlgorithm.decrypt(encryptBytes, KeyType.PrivateKey);//解密
...
//            log.debug(KrestUtil.getTempSecretKey());
            ...
            SymmetricCrypto messageBodyCryptoAlgorithm=krestConfigurer.createMessageBodyCryptoAlgorithm(tempSecretKey);
            Assert.notNull(messageBodyCryptoAlgorithm,"您尚未实现krestConfigurer.createMessageBodyCryptoAlgorithm(tempSecretKey)接口");
